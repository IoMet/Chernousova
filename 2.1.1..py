...
class ResultStatic:
    ...
    @staticmethod
    def check_one_percent(salaries, counts):
...
class Report:
    ...
    def __init__(self, vac_name, list_by_year, list_by_area):
        ...
        self.generate_excel(vac_name, list_by_year, list_by_area)

    def generate_excel(self, vac_name, list_by_year, list_by_area):
        thins = Side(border_style="thin", color="000000")

        self.set_value_page(vac_name, list_by_year, list_by_area, thins)
        self.set_width(thins)

        self.book.save("report.xlsx")

    def set_value_page(self, vac_name, list_by_year, list_by_area, thins):
        # Для первого листа
        for i, value in enumerate(self.rows_by_year, 1):
            self.page_by_year.cell(row=1, column=i).value = value + vac_name if " - " in value else value
...
class PrepareVacData:
    def __init__(self):
        ...
        report = Report(inputed.vacancy_name, result.list_by_year, result.list_by_area)
