...
def multiprocessing(vacancy, year, process_queue):
...
if __name__ == "__main__":
    ...
    all_processes = []
    for year in years:
        p = Process(target=multiprocessing, args=(vacancy_name, year, queue))
        all_processes.append(p)
        p.start()

    for process in all_processes:
        returned_list = queue.get()
